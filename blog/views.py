...
from .models import Post, Category, Comment, Gallery, Site, Skill, SubScriber, Contact, create_slug
...
class LoginUser(LoginView):
    ...
    def get_success_url(self):
        ...
        messages.success(self.request, f"Welecom my doba")
...
class HomeView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        popular_posts = self.object_list.annotate(
            num_likes=Count('like')).order_by('-num_likes')[:10]
...
def UpdateCommentView(request, slug):
    comment_form = CommentForm(data=request.POST)
    if comment_form.is_valid():
        post_id = comment_form.data.get("post_id")
        parent_id = comment_form.data.get("parent_id")
    ...
    if request.method == 'POST':
        ...
        if name and from_email and senderMessage and senderphone:
            Contact.objects.create(
                name=name, phone=senderphone, email=from_email, message=senderMessage)
        else:
...
class PostCreateView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        # 1번문제 editor내용 뺴내기
        # 2번문제 벨리데이션 후 처리 class Base라면 방법이 내부에 있을듯
        # 3번문제
